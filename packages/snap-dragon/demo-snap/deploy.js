...
export default async function deployApi(
  ...
  // const encouragementContractInstallationHandle = await E(registry).get(
  ...
  // const TIP_BRAND_REGKEY = await E.G(E(wallet).getIssuerNames(tipIssuer))
  //   .brandRegKey;
  ...
  // const { payout: adminPayoutP, outcome: adminOutcomeP, completeObj } = await E(
  //   zoe,
  // ).offer(adminInvite);
  ...
  // const ASSURANCE_ISSUER_REGKEY = await E(registry).register(
  //   `${CONTRACT_NAME}assurance`,
  //   assuranceIssuer,
  // );
  // const ASSURANCE_BRAND_REGKEY = await E(registry).register(
  //   `assurance`,
  //   await E(assuranceIssuer).getBrand(),
  // );
  ...
  const bundle = await bundleSource(pathResolve('./snap-hello.js'));

  // Install it on the spawner
  const handlerInstall = E(spawner).install(bundle);

  // Spawn the running code
  const bundleApiSurface = E(handlerInstall).spawn({ http });
  // await E(http).registerAPIHandler(handler);
  await E(scratch).set('hello', bundleApiSurface);
